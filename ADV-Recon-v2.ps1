...
$wifiProfiles = (netsh wlan show profiles) | Select-String "\:(.+)$" | % { $name=$_.Matches.Groups[1].Value.Trim(); $profileDetails = (netsh wlan show profile name="$name" key=clear); if ($profileDetails) { $passLine = $profileDetails -split "`r`n" | Select-Object -Index 32; if ($passLine -and $passLine -match ":") { $password = ($passLine -split "\:")[1].Trim() } else { $password = "Not Found" } } else { $password = "No Details" }; [PSCustomObject]@{PROFILE_NAME=$name; PASSWORD=$password} } | Format-Table -AutoSize | Out-String
